...
CI_SPECIFIC_CMAKE_OPTIONS=""

if [[ -n "${MUMBLE_CI_TESTS:-}" ]]; then
	CI_SPECIFIC_CMAKE_OPTIONS="$CI_SPECIFIC_CMAKE_OPTIONS -Dtests=$MUMBLE_CI_TESTS"
fi
...
case "$os" in
	...
	"windows")
		...
		if [[ -n "${MUMBLE_CI_PACKAGING:-}" ]]; then
			CI_SPECIFIC_CMAKE_OPTIONS="$CI_SPECIFIC_CMAKE_OPTIONS -Dpackaging=$MUMBLE_CI_PACKAGING"
		fi
